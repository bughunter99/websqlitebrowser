...
def settings_test_view(request: HttpRequest) -> JsonResponse:
	try:
		...
		settings_data = {
			...
			'http_referer': str(payload.get('http_referer', '')).strip() or stored_settings.get('http_referer', ''),
			'x_title': str(payload.get('x_title', '')).strip() or stored_settings.get('x_title', ''),
			'user_agent': str(payload.get('user_agent', '')).strip() or stored_settings.get('user_agent', ''),
